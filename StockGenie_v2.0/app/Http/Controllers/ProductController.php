...
class ProductController extends Controller
{
    ...
    //             'buying_price' => $request->buying_price,
    //             'selling_price' => $request->selling_price,
    //             'updated_by'    => auth()->id(),// Store who updated the product
    ...
    public function updateProductQtyPrice(Request $request, $id) 
{
    ...
    // নতুন মান (nullable হতে পারে)
    $new_qty          = (int) $request->updateQty;
    $new_buying_price = $request->buying_price !== null ? (float) $request->buying_price : null;
    $new_selling_price= $request->selling_price !== null ? (float) $request->selling_price : null;

    // Quantity cumulative হবে
    $updated_qty = $old_qty + $new_qty;

    // Update data প্রস্তুত করা
    $updateData = [
        'sup_id'       => $request->supplier_id,
        'product_qty'  => $updated_qty,
        'buying_price' => $old_buying_price,   // default পুরনো দাম
        'selling_price'=> $old_selling_price,  // default পুরনো দাম
    ];

    // Condition অনুযায়ী দাম override হবে
    if ($new_buying_price == null) {
        $updateData['buying_price'] = $old_buying_price;
    }else {
        $updateData['buying_price'] = $new_buying_price;
    }
    if ($new_selling_price == null) {
        $updateData['selling_price'] = $old_selling_price;
    }else {
        $updateData['selling_price'] = $new_selling_price;
    }

    // Validation: বিক্রয়মূল্য সবসময় ক্রয়মূল্যের থেকে বেশি হতে হবে
    // if ($updateData['selling_price'] < $updateData['buying_price']) {
    //     return redirect()->route('updateProductQtyView')->with([
